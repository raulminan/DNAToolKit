"""distances functions for DNAToolKit"""
import utils
...
def neighbors(pattern: str, d: int) -> list:
    ...
    neighborhood = set()
    suffix_neighbors = neighbors(utils.suffix(pattern), d)
    for text in suffix_neighbors:
        if hamming_distance(utils.suffix(pattern), text) < d:
            for nucleotide in "ATCG":
                neighborhood.add(nucleotide + text)
        else:
            neighborhood.add(utils.first_symbol(pattern) + text)
